...
class Collect(object):
    ...
    def _collect_dict(self, data_dict):
        """
        处理单个字典的核心逻辑
        """
        data = dict()
        ...
        for name, sub_keys in self.kwargs.items():
            name = name.replace("_keys", "")
            assert isinstance(sub_keys, Sequence)
            data[name] = np.concatenate([data_dict[key].astype(np.float32) for key in sub_keys], axis=1)
        return data

    def __call__(self, data):
        """
        根据输入类型（单个字典或字典列表）分派操作。
        """
        if isinstance(data, str):
            return data

        elif isinstance(data, Sequence):
            return [self._collect_dict(item) for item in data]

        elif isinstance(data, Mapping):
            return self._collect_dict(data)

        else:
            raise TypeError(f"Collect: 不支持的数据类型 {type(data)}")
    ...
    def __call__(self, data_dict):
        if "color" in data_dict:
            ...
            if isinstance(color, np.ndarray):
                ...
                if max_val > 1.0:
                    data_dict["color"] = color.astype(np.float32) / 255.0
                    
            elif isinstance(color, torch.Tensor):
                if color.numel() == 0:
                    return data_dict
                max_val = torch.max(color)
                if max_val > 1.0:
                    data_dict["color"] = color.float() / 255.0
...
class GridSample(object):
    ...
    def _count_voxels(self, coord):
        scaled_coord = coord / np.array(self.grid_size)
        grid_coord = np.floor(scaled_coord).astype(int)
        min_coord = grid_coord.min(0)
        grid_coord -= min_coord
        key = self.hash(grid_coord)
        unique_keys, inverse, count = np.unique(key, return_inverse=True, return_counts=True)
        return unique_keys.shape[0], inverse, count

    def _get_grid_properties(self, data_dict):
        ...
        origin_inverse = np.zeros_like(inverse)
        origin_inverse[idx_sort] = inverse
    ...
    def get_sample_indices(self, data_dict):
        properties = self._get_grid_properties(data_dict)
    ...
    def __call__(self, data_dict):
        ...
        scaled_coord = properties["scaled_coord"]
        grid_coord = properties["grid_coord"]
        min_coord = properties["min_coord" ]
